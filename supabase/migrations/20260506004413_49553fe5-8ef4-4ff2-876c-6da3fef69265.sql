...
-- 1. New date fields on equipment
ALTER TABLE public.equipment
  ADD COLUMN IF NOT EXISTS last_hydrostatic_test date,
  ADD COLUMN IF NOT EXISTS next_hydrostatic_test date,
  ADD COLUMN IF NOT EXISTS last_calibration date,
  ADD COLUMN IF NOT EXISTS next_calibration date;

-- 2. Per-category configurable auto-rejection
ALTER TABLE public.categories
  ADD COLUMN IF NOT EXISTS blocking_expiries jsonb NOT NULL DEFAULT '["certificate_expiry","expiry_date"]'::jsonb;

-- 3. Equipment relationships (many-to-many with type)
CREATE TABLE IF NOT EXISTS public.equipment_relationships (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  parent_equipment_id uuid NOT NULL REFERENCES public.equipment(id) ON DELETE CASCADE,
  child_equipment_id uuid NOT NULL REFERENCES public.equipment(id) ON DELETE CASCADE,
  relationship_type text NOT NULL DEFAULT 'component',
  notes text,
  created_by uuid,
  created_at timestamp with time zone NOT NULL DEFAULT now(),
  CONSTRAINT equipment_relationship_unique UNIQUE (parent_equipment_id, child_equipment_id, relationship_type),
  CONSTRAINT equipment_relationship_no_self CHECK (parent_equipment_id <> child_equipment_id)
);

CREATE INDEX IF NOT EXISTS idx_equipment_relationships_parent ON public.equipment_relationships(parent_equipment_id);
CREATE INDEX IF NOT EXISTS idx_equipment_relationships_child ON public.equipment_relationships(child_equipment_id);

ALTER TABLE public.equipment_relationships ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Users can view relationships from their ships"
ON public.equipment_relationships
FOR SELECT
USING (
  EXISTS (
    SELECT 1 FROM public.equipment e
    WHERE e.id = equipment_relationships.parent_equipment_id
      AND public.user_has_ship_access(auth.uid(), e.ship_id)
  )
  AND EXISTS (
    SELECT 1 FROM public.equipment e
    WHERE e.id = equipment_relationships.child_equipment_id
      AND public.user_has_ship_access(auth.uid(), e.ship_id)
  )
);

CREATE POLICY "Admins and technicians can insert relationships"
ON public.equipment_relationships
FOR INSERT
WITH CHECK (
  public.is_admin_or_technician(auth.uid())
  AND EXISTS (
    SELECT 1 FROM public.equipment e
    WHERE e.id = equipment_relationships.parent_equipment_id
      AND public.user_has_ship_access(auth.uid(), e.ship_id)
...
CREATE POLICY "Admins and technicians can update relationships"
ON public.equipment_relationships
FOR UPDATE
USING (
  public.is_admin_or_technician(auth.uid())
...
CREATE POLICY "Admins and technicians can delete relationships"
ON public.equipment_relationships
FOR DELETE
USING (
  public.is_admin_or_technician(auth.uid())
...
-- 4. Audit log trigger for equipment_relationships
CREATE OR REPLACE FUNCTION public.log_equipment_relationship_changes()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  IF TG_OP = 'INSERT' THEN
    ...
    VALUES ('equipment_relationships', NEW.id, 'INSERT', to_jsonb(NEW), v_user_id, v_user_name);
    RETURN NEW;
  ELSIF TG_OP = 'DELETE' THEN
    INSERT INTO public.audit_logs (table_name, record_id, action, old_data, user_id, user_name)
    VALUES ('equipment_relationships', OLD.id, 'DELETE', to_jsonb(OLD), v_user_id, v_user_name);
    RETURN OLD;
  END IF;
  RETURN NULL;
END;
...
CREATE TRIGGER equipment_relationships_audit
AFTER INSERT OR DELETE ON public.equipment_relationships
FOR EACH ROW EXECUTE FUNCTION public.log_equipment_relationship_changes();

-- 5. Update get_dashboard_stats to consider configurable blocking expiries + new dates in alerts
CREATE OR REPLACE FUNCTION public.get_dashboard_stats(p_ship_id uuid DEFAULT NULL::uuid)
 RETURNS json
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
BEGIN
  ...
  -- Auto-rejected based on per-category blocking_expiries
  ...
  WHERE (p_ship_id IS NULL OR e.ship_id = p_ship_id)
    AND (
      e.status = 'rejected'
      OR (coalesce(c.blocking_expiries, '["certificate_expiry","expiry_date"]'::jsonb) ? 'certificate_expiry'
          AND e.certificate_expiry IS NOT NULL AND e.certificate_expiry::date < v_today)
      OR (coalesce(c.blocking_expiries, '["certificate_expiry","expiry_date"]'::jsonb) ? 'expiry_date'
          AND e.expiry_date IS NOT NULL AND e.expiry_date::date < v_today)
      OR (coalesce(c.blocking_expiries, '[]'::jsonb) ? 'next_hydrostatic_test'
          AND e.next_hydrostatic_test IS NOT NULL AND e.next_hydrostatic_test::date < v_today)
      OR (coalesce(c.blocking_expiries, '[]'::jsonb) ? 'next_calibration'
          AND e.next_calibration IS NOT NULL AND e.next_calibration::date < v_today)
    );
  ...
  WHERE (p_ship_id IS NULL OR e.ship_id = p_ship_id)
    AND e.status = 'active'
    AND NOT (coalesce(c.blocking_expiries, '["certificate_expiry","expiry_date"]'::jsonb) ? 'certificate_expiry'
             AND e.certificate_expiry IS NOT NULL AND e.certificate_expiry::date < v_today)
    AND NOT (coalesce(c.blocking_expiries, '["certificate_expiry","expiry_date"]'::jsonb) ? 'expiry_date'
             AND e.expiry_date IS NOT NULL AND e.expiry_date::date < v_today)
    AND NOT (coalesce(c.blocking_expiries, '[]'::jsonb) ? 'next_hydrostatic_test'
             AND e.next_hydrostatic_test IS NOT NULL AND e.next_hydrostatic_test::date < v_today)
    AND NOT (coalesce(c.blocking_expiries, '[]'::jsonb) ? 'next_calibration'
             AND e.next_calibration IS NOT NULL AND e.next_calibration::date < v_today);
  ...
  FROM (
    ...
    SELECT jsonb_build_object(
      'id', 'alert-hydro-expiring-' || e.id,
      'type', 'expiring',
      'message', 'Teste hidrostático vencendo',
      'messageKey', 'alerts.msgHydroTestExpiring',
      'equipmentId', e.id,
      'equipmentName', e.name || ' ' || e.internal_code,
      'date', e.next_hydrostatic_test,
      'severity', 'medium'
    ) as alert
    FROM equipment e
    WHERE (p_ship_id IS NULL OR e.ship_id = p_ship_id)
      AND e.next_hydrostatic_test IS NOT NULL
      AND e.next_hydrostatic_test::date >= v_today
      AND e.next_hydrostatic_test::date <= v_thirty_days

    UNION ALL

    SELECT jsonb_build_object(
      'id', 'alert-calibration-expiring-' || e.id,
      'type', 'expiring',
      'message', 'Calibração vencendo',
      'messageKey', 'alerts.msgCalibrationExpiring',
      'equipmentId', e.id,
      'equipmentName', e.name || ' ' || e.internal_code,
      'date', e.next_calibration,
      'severity', 'medium'
    ) as alert
    FROM equipment e
    WHERE (p_ship_id IS NULL OR e.ship_id = p_ship_id)
      AND e.next_calibration IS NOT NULL
      AND e.next_calibration::date >= v_today
      AND e.next_calibration::date <= v_thirty_days
